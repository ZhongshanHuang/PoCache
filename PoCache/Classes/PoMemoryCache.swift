...
// MARK: - PoMemoryCache
public final class PoMemoryCache<Key, Value> where Key : Hashable {
    ...
    public var didReceiveMemoryWarningBlock: ((PoMemoryCache) -> Void)?

    /// The block will be executed when the app enter background.
    public var didEnterBackgroundBlock: ((PoMemoryCache) -> Void)?

    /// The key-value pair will be released on the main thread if true. otherwise on the background thread.
    public var isReleasedOnMainThread: Bool = false {
        didSet {
    ...
    public var isReleasedAsynchronously: Bool = true {
        didSet {
    ...
    public func setObject(_ anObject: Value, forKey key: Key, cost: Int = 0) {
        pthread_mutex_lock(&_lock)
